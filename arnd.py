...
class TheBrick(GameObject):
    ...
    def __init__(self, canvas, x, y, hits):
        ...
        self.height = 20
        self.hits = hits
        color = TheBrick.COLORS[hits]
        item = canvas.create_rectangle(x-self.width/2, y-self.height/2,
                                  x+self.width/2, y+self.height/2,
                                  fill=color, tag="brick")

        super(TheBrick, self).__init__(canvas, item)
    ...
    def hit(self):
        ...
        if self.hits == 0:
            ...
            self.canvas.itemconfig(self.item, fill=TheBrick.COLORS[self.hits])
...
class Game(tk.Frame):
    ...
    def add_ball(self):
        ...
        self.ball = TheBall(self.canvas, x, 310)
        self.paddle.set_ball(self.ball)

    # add brick in game
    def add_brick(self, x, y, hits):
        brick = TheBrick(self.canvas, x, y, hits)
    ...
    def __init__(self, master):
        ...
        self.items = {}
        self.ball = None
        self.paddle = ThePaddle(self.canvas, self.width/2, 326)
        self.items[self.paddle.item] = self.paddle

        for i in range(5, self.width - 5, 75):
            self.add_brick(i + 37.5, 50, 3)
            self.add_brick(i + 37.5, 70, 2)
